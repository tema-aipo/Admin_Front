...
import type { ConversationLog, QuestionCategory, ChatbotStats } from "../../types";
...
export function ChatbotManagement() {
  ...
  //     try {
  //       const [logs, categories, statistics] = await Promise.all([
  //         fetch('/api/admin/chatbot/logs').then(r => r.json()),
  //         fetch('/api/admin/chatbot/categories').then(r => r.json()),
  //         fetch('/api/admin/chatbot/stats').then(r => r.json()),
  //       ]);
  ...
  // 낮은 평가(싫어요) 필터링
  const lowRatedConversations = conversationLogs.filter(
        ...
        <Tabs defaultValue="all">
            ...
            {lowRatedConversations.length === 0 ? (
              ...
              <div className="space-y-3">
                {lowRatedConversations.map((log) => (
                  <div
                    key={log.id}
                    className="p-5 bg-red-50 rounded-lg border border-red-200 hover:shadow-md transition-all duration-200"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                        ...
                        <div className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-red-200">
          ...
          <TabsContent value="categories">
            {questionCategories.length > 0 ? (
              ...
              {/* 바 차트 */}
              <div className="bg-gradient-to-br from-white to-blue-50 rounded-lg border border-gray-200 p-6">
              ...
              {/* 파이 차트 */}
              <div className="bg-gradient-to-br from-white to-purple-50 rounded-lg border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
